...
def main():
    ...
    st.title("Extract Copernicus Data")
    ...
    if latitude!=0 and longitude!=0 and st.session_state.button_pressed:
        ...
        formated_current_time = current_time.strftime('%Y_%m_%d')
        ...
        print("The request is going to start")
    ...
    if end_of_request:
        ...
        if dataset == "derived-near-surface-meteorological-variables":
            df=put_zip_in_csv(folder_to_create)
        else : 
            df=put_nc_in_csv(folder_to_create)

        df.to_csv(path_or_buf=f'{folder_to_create}/{start_year}_{stop_year}.csv')

   
        # This allows to make the map appear again, the parameters as well.
